...
def get_tiny_images(image_paths):
    ...
    for i in image_paths:
        ...
        temp = np.append(temp, image , axis = 0)
    feats = temp[1:]
    return feats
...
def build_vocabulary(image_paths, vocab_size):
    ...
    for i in image_paths:
        image = load_image_gray(i)
        #descriptors N*128
        frames, descriptors = vlfeat.sift.dsift(image,fast=True,step=25,size=50)
...
def get_bags_of_sifts(image_paths, vocab_filename):
    ...
    for i in image_paths:
        image = load_image_gray(i)
        frames, descriptors = vlfeat.sift.dsift(image,step = 3, size = 8, fast=True, float_descriptors=True)
        ...
        hist, bin_edge = np.histogram(assignments,range(k+1))
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats,
    ...
    """
    test_labels = []
    distances = sklearn_pairwise.pairwise_distances(test_image_feats, train_image_feats)
    for i in distances:
        idx = np.argmin(i)
    ...
    for s in svms:
        ...
        for i in train_labels:
            if s==i:
                ...
                train_set.append(-1)
        svms[s].fit(train_image_feats,train_set)
